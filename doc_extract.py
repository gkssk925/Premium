...
def main():
    ...
    for i in range(len(cond_matched_mhz)):
        ...
        if any(lowest in s for s in words):
            cond_matched_mhz[i] = cond_matched_mhz[i].replace('Lowest', str(min_mhz[i])+'MHz')
        if any(highest in s for s in words):
            cond_matched_mhz[i] = cond_matched_mhz[i].replace('Highest', str(max_mhz[i])+'MHz')
        if any(word_all in s for s in words):
            print("line num=======================", i)
            cond_matched_mhz[i] = cond_matched_mhz[i].replace('All', extracted_mhz[i])
